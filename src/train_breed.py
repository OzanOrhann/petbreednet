...
def loaders(root,img=224,bs=32,w=4):
    ...
    return (DataLoader(tr,bs,True,w,pin_memory=True),
            DataLoader(va,bs,False,w,pin_memory=True),
            tr.classes)
...
def train_one(m,ld,opt,sc,dev,crit):
    ...
    for x,y in ld:
        ...
        with torch.cuda.amp.autocast(enabled=True):
            o=m(x); l=crit(o,y)
        sc.scale(l).step(opt); sc.update()
        L+=l.item()*x.size(0); A+=acc(o,y)*x.size(0); N+=x.size(0)
    return L/N, A/N
